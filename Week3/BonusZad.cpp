...
void insert(std::fstream& file)
{
    ...
    //cout << file.fail() << "OR" << file.bad() <<"OR" << file.eof() << endl;
    //file.flush();
    //file.clear();
    //file.ignore(1024,'\n');
    //cout << file.fail()<< "OR" <<file.bad()<<endl; DEBUG SHIT
    selectionSort(file);
...
void StartUp()
{
    ...
    handler.file.open(filename, std::ios::out | std::ios::in | std::ios::binary);
    ...
    while (true)
    {
        ...
        else if (choice == 2)
        {
            ...
            getPokemonAndPrint(handler.file);

        }
        else if (choice == 3)
        {
            printAllPokemons(handler.file);
        }
        else if (choice == 4)
        {
            ...
            cin >> filenameToSave;
            textify(handler.file, filenameToSave);
        }
        else if (choice == 5)
        {
            ...
            cin >> fileNameToOpen;
            if (untextify(handler.file, fileNameToOpen))
            {
                handler.file.open(filename, std::ios::in | std::ios::out | std::ios::binary);
            }
        }
        else {
